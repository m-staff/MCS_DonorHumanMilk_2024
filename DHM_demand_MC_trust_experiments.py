...
for MCS in range (1, MCS_iterations+1):
    ...
    for tt in range(len(thelist)):
        ...
        total_per_Trust=0
        max_cluster = math.floor((max(Y,Z)-20)/3)
        for cluster in range(1,max_cluster+1):
            ...
            for delivery in range(1,n_of_deliveries+1): #split between weeks and singl-twins
                ...
                for baby_born in range(1, n_of_babies+1):
                    ...
                    for i in range(1, 38-GA+1):
                        day_step=(weight_array[i]-weight_array[i-1])/7
...
percentiles = np.percentile(results, [5,10,25,50,75,90,95], axis =1)
...
df_percentiles=pd.DataFrame(percentiles, columns=["5","10","25", "50","75", "90", "95"])
df_results_individual=pd.DataFrame(results, )

#to merge 2 dfs
df_final_results=pd.concat([df_means, df_percentiles, df_results_individual], axis=1)

#code to append to existing Excel file: if it does not exist, please create it
with pd.ExcelWriter('Trust_Experiments.xlsx', mode='a', engine='openpyxl', if_sheet_exists='replace') as writer:
